...
# -*- coding: utf-8 -*-
"""
The main functions are find2c, hselect, uniform, rvbina, rvextract, setrvs, spbina, splot, vexplore and vgrid. 
...
def help(function):
    if function.lower() == 'find2c':
        ...
        companion for each mass ratio grid defined by arguments (qmin, qmax and deltaq). The name of observed \
        spectra must be provide in a text file and the character "@" must go before the file name. The \
        match solution for mass ratio and effective temperature for secondary companion is made by cross-correlation \
        of each secondary spectrum with spectra template. The output shows a 3D graphic of cross-correlaton \
        function and mass ratio and effective temperature for the best solution.\n')
        ...
        print('    vgamma - estimated value for systemic radial velocity in km/s\n')
        print('Optional arguments:')
        print('       spa - output name of primary component mean spectrum')
        print('       spb - output name of secondary component mean spectrum')
        print('      qmin - minimum mass ratio for cross-correlation grid')
        print('      qmax - maximum mass ratio for cross-correlation grid')    
        print('    deltaq - mass increments for cross correlation grid')
        print('      wreg - spectral regions for cross correlation analysis')
        print('     nproc - number of CPU cores to use in computing\n')
    elif function.lower() == 'hselect':
        print('\nHSELECT\n\nExtract keyword values from images FITS or file list. For file list the character \
        "@" must go before the file name.\n')
        print('Mandatory arguments:')
        ...
        method (FFT) to perform the convolution. If the input is a file list of observed spectra, the character \
        "@" must go before the file name. The output is the full discrete linear convolution of the inputs with \
        a dispersion grid defined by spectral region argument.\n')
        print('Mandatory arguments:')
        print('       lis - list of observed spectra to process (use @ for file list)\n')
        print('Optional arguments:')
        print('       spa - name of primary component mean spectrum')
        print('       spb - name of secondary component mean spectrum')
        print('        ta - spectrum template to use as primary component')
        print('        tb - spectrum template to use as secondary component')
        print('      wreg - spectral regions for cross correlation analysis')
        print('     aconv - damping convergence factor')
        print('     keyjd - header keyword that contains Julian Date')
        print('   fitcont - continuum subtract the spectra prior to correlation analysis\n')
        print('   interac - process cross-correlation interactively (interactive mode)\n')
        ...
        print('\nSETRVS: measurement radial velocities for each spectrum from dataset.\n')
        print('Mandatory arguments:')
        print('       lis - list of observed spectra to process (use @ for file list)\n')
        print('Optional arguments:')
        print('        ta - spectrum template to use as primary component')
        ...
        print('      frat - estimated flux ratio among components')
        ...
        print('Optional arguments:')
        print('      xmin - lower wavelength limit for graphic')
        print('      xmax - upper wavelength limit for graphic')
        print('      ymin - lower flux limit for graphic')
        print('      ymax - upper flux limit for graphic')
        print('     scale - flux scale factor')
        print('   markpix - mark pixel values')
        print('    newfig - open spectrum in a new window') 
        print('     color - spectrum graphic color') 
    elif function.lower() == 'uniform':
        print('\nUNIFORM: scale each spectrum from dataset to a mean continuum factor.\n')
        print('Mandatory arguments:')
        print('       lis - list of observed spectra to process (use @ for file list)\n')
        print('Optional arguments:')
        print('   interac - show corrected spectra (interactive mode)\n')
    elif function.lower() == 'vexplore':
        print('\nVEXPLORE: explore results for systemic radial velocities grid analysis\n')
        print('Mandatory arguments:')
        print('    folder - folder output name (obtained from vgrid task)')
        ...
        print('       lit - full path to the folder containing spectra templates')
        print('Optional arguments:')
        print('     svmin - lower systemic radial velocity for grid in km/s')
        print('     svmax - upper systemic radial velocity for grid in km/s')
        print('      step - radial velocity step for meshing grid in km/s')
        print('      qmin - minimum mass ratio for cross-correlation grid ')
        print('      qmax - maximum mass ratio for cross correlation grid')    
        print('    deltaq - mass incrememnts forcross correlation grid')
        print('      wreg - spectral regions for cross correlation analysis')
...
def find2c(lis, lit, vgamma, spa='A', spb='B', qmin=0.02, qmax=0.5, deltaq=0.01, 
    ...
    with Pool(processes=nproc) as pool:
        ...
        pres = [pool.apply_async(qparallel, args=[chk, lis, spa, spb, deltaq, vgamma]) 
                for chk in qa2]
...
def vgrid(lis, lit, svmin=-1, svmax=1, step=0.1, qmin=0.02, qmax=0.5, deltaq=0.01, 
    ...
    for vgamma in svrange:
        ...
        with Pool(processes=nproc) as pool:
            qa2 = np.array_split(q_array, nproc)
            pres = [pool.apply_async(qparallel, args=[chk, lis, 'A', 'B', deltaq, vgamma]) 
                    for chk in qa2]
            pres2 = [chk.get() for chk in pres]
...
def uniform(lis, wmin=None, wmax=None, keyrv='VRA', cmcrem=True, nit=5, lo=5, hi=2, interac=True):
    ...
    for i, img in enumerate(larch2):
        ...
        #recortar si los parametros los piden
        if wmin is not None:
        ...
        print('')
        print('\tSaving ' + img + '...')
        # Crear un header nuevo con solo los keywords esenciales
        header_nuevo = fits.Header()
        # Lista de keywords esenciales que quieres conservar
        keywords_esenciales = [
            'SIMPLE', 'BITPIX', 'NAXIS', 'NAXIS1', 'EXTEND', 'OBJECT', 'CRPIX1',
            'CRVAL1', 'CDELT1',  'CTYPE1', 'BUNIT','CUNIT1', 'RA', 'DEC', 'EQUINOX', 'DATE-OBS', 'MJD-OBS', 'EXPTIME', 'TELESCOP', 'INSTRUME',
            'RADECSYS', 'ORIGIN', 'DATE', 'AIRMASS', 'UTC', 'LST', 'HJD', 'CD1_1', keyrv]
        
        # Copiar solo los keywords esenciales que existen y son seguros
        for kw in keywords_esenciales:
            if kw in hobs[0].header:
                try:
                    value = hobs[0].header[kw]
                    # Si el valor es string, asegurar que es ASCII
                    if isinstance(value, str):
                        value = ''.join(char for char in value if ord(char) < 128)
                    header_nuevo[kw] = value
                except:
                    pass  # Si falla, no incluir este keyword
        
        # Establecer los NAXIS correctamente para el espectro 1D
        header_nuevo['NAXIS'] = 1  # Espectro 1D
        header_nuevo['NAXIS1'] = len(new_disp_grid[i_min:i_max])
        
        # Configurar parámetros de la grilla lineal
        header_nuevo['CRVAL1'] = new_disp_grid[i_min]  # Longitud de onda inicial
        header_nuevo['CRPIX1'] = 1.0  # Pixel de referencia
        header_nuevo['CTYPE1'] = 'WAVELEN'  # Tipo de eje
        header_nuevo['CUNIT1'] = 'Angstrom'  # Unidades (ajustar según corresponda)
        
        # Guardar el espectro con header simplificado
